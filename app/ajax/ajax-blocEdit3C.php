    ...
    if (!empty($vin)) {
        ...
        if (!empty($_FILES)) {
            ...
            if (!in_array($ext, $valid_extensions)) {
                ...
                array_push($attachement, $_SERVER['DOCUMENT_ROOT'] . '/assets/img/domaine_images/' . $file_name);
                array_push($attachements, 'assets/img/domaine_images/' . $file_name);

                move_uploaded_file($fichier['file_0']['tmp_name'], $_SERVER['DOCUMENT_ROOT'] . '/assets/img/domaine_images/' . $file_name);
            }
            ...
            if ($final['success']) {
                ...
                $attachemens = str_replace('assets/img/domaine_images/', '', $attachemens);

                $insert = $db->query('INSERT INTO `vin_domaine_images`(
                        `vin_id`,
                        `value`,
                        `created_at`
                        ) VALUES (
        ...
        `content` = "' . stripslashes($_POST['content_fr']) . '"
         WHERE lang = "fr" AND vin_id = "' . $_POST['vin_id'] . '"');

        $update = $db->query('UPDATE `definitions` SET 
         `content` = "' . stripslashes($_POST['content_en']) . '"
          WHERE lang = "en" AND vin_id = "' . $_POST['vin_id'] . '"');
