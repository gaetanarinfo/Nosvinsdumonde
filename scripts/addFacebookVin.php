...
$selects = selectDB('nom, id, created_at, boisson_id', 'vins', 'publish_facebook != 1 ORDER BY created_at', $db, '*');
...
foreach ($selects as $value) {
    ...
    $status = [
        'link' => 'https://nosvinsdumonde.com/fr/' . $url . '/' . $value['id'],
        'message' => 'À découvrir sur Nosvinsdumonde 🍷 ' . html_entity_decode($value['nom']),
    ];

    postStatus($status);

    $update = $db->query('UPDATE `vins` SET `publish_facebook` = 1 WHERE id = "' . $value['id'] . '"');
}
